	...
	for mod, ns := range nodes {
		...
		for _, n := range ns {
			...
			data[mod][n.Level] = append(data[mod][n.Level], map[string]interface{}{
				...
				"position": "https://github.com/zhengyansheng/jupiter/blob/master/" + filepath + "#L" + strconv.Itoa(n.Position.Line),
			})
		}
	}
	...
	for _, desc := range configDescs {
		...
		if _, ok := configMap[desc.Package]; ok {
			...
			if desc.Name != "" {
				...
				mdData["position"] = "https://github.com/zhengyansheng/jupiter/blob/master/" + filepath + "#L" + strconv.Itoa(desc.Position.Line)
			}
...
> 本文档是由github.com/zhengyansheng/jupiter/script/ast_codes自动生成   
